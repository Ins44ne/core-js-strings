...
function removeLastOccurrences(str, value) {
  let strPart1 = '';
  let strPart2 = '';
  const foundIndex = str.lastIndexOf(value);
  if (foundIndex === -1) {
    return str;
  }
  strPart1 = str.slice(0, foundIndex);
  strPart2 = str.slice(foundIndex + value.length);
...
function formatTime(minutes, seconds) {
  const minutesNew = String(minutes).padStart(2, '0');
  const secondsNew = String(seconds).padStart(2, '0');
...
function isPalindrome(str) {
  ...
  for (let i = 0; i <= strNew.length - 1; i += 1) {
...
function findLongestWord(sentence) {
  let result = '';
  let maxLength = 0;
  const wordsString = sentence.replace(/[,]/g, '').split(' ');
  wordsString.forEach((item) => {
...
function unbracketTag(str) {
  let strNew = '';
  const positionLeft = str.indexOf('<');
  if (positionLeft !== -1) {
    strNew = str.slice(0, positionLeft) + str.slice(positionLeft + 1);
  }
  const positionRight = strNew.indexOf('>');
  if (positionRight !== -1) {
    strNew = strNew.slice(0, positionRight) + strNew.slice(positionRight + 1);
  }
  return strNew;
}
...
function extractEmails(str) {
  const seps = [' ', ',', ';', ':', '\n'];
  const words = str.split(new RegExp(`[${seps.join('[')}]`));
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (letter) => {
    const offset = letter <= 'Z' ? 65 : 97; // Определяем смещение для заглавных и строчных букв
    const letterCode = letter.charCodeAt(0);
    const rotatedletterCode = ((letterCode - offset + 13) % 26) + offset;
    return String.fromCharCode(rotatedletterCode);
  });
}
